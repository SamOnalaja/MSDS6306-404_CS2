...
##Executive Summary
#### Talent management is an iterative process of developing and retaining employees. Our analysis yielded that there are ten (10) factors that contribute to turnover with an overall 87.3% accuracy. While there were positive relationships between the varying satisfaction metrics recently collected by job roles, the differences were not statistically significant.
...
####The mean relationship satisfaction for all job roles ranged from 2.50 to 2.77. Albeit at a p-value 0.9362, there is not sufficient evidence to suggest that the difference is statistically significant between the roles. Managers have the highest mean relationship satisfaction with 33% reported as Very High whereas Sales Representatives have the lowest mean with a reported 26% Low to and 28% High.
...
####With an 87.3% overall accuracy, our custom regression model identified that business travel, commute distance, environment satisfaction, job involvement, job role, job satisfaction, marital status, number of companies worked, overtime, and relationship satisfaction are factors that contribute to attrition. If instead one wishes for a slightly less complex model, another one was created with 85% accuracy. This model contains half the number of terms (five): job involvement, overtime, marital status, job role, and number of companies worked for. Managing these factors in developing and retaining employees could reduce and prevent attrition.
...
#cust.mat$table
```
```[Chart 6: Accuracy and #Terms]```

```{r writeit, echo=FALSE, warning=FALSE, message=FALSE, comment=NA }
...
#knn.model2 <- class::knn(train_transformed, test_tranformed, dfTrain.train$Attrition, k=34)

#Returns "Error in class::knn(dfTrain.train[, c(3, 15, 17, 19, 22, 24)], dfTrain.test[,  : NA/NaN/Inf in foreign function call (arg 6)"

#dfTrain.test$knnpred <- factor(knn.model2)
#confusionMatrix(table(dfTrain.test$Attrition,dfTrain.test$knnpred))



#KNN using caret and mlr packages where k=34 from sqrt(1170)

#knn.model <- knnreg(Attrition~JobInvolvement + OverTime + MaritalStatus + JobRole + NumCompaniesWorked, data = dfTrain.train,k=34)
#dfTrain.test$knn <- predict(knn.model,dfTrain.test)
#confusionMatrix(table(dfTrain.test$knn,dfTrain.test$Attrition))

#Returns "Error in !all.equal(nrow(data), ncol(data)) : invalid argument type"



#Converting every categorical variable to numerical using dummy variables
dmy.train <- dummyVars(" ~ .", data = dfTrain.train,fullRank = T)
train_transformed <- data.frame(predict(dmy.train, newdata = dfTrain.train))
str(train_transformed)

dmy.test <- dummyVars(" ~ .", data = dfTrain.test,fullRank = T)
test_transformed <- data.frame(predict(dmy.test, newdata = dfTrain.test))
str(test_transformed)

knn.model <- knnreg(train_transformed, train_transformed$Attrition, k=34)
test_transformed$knn <- predict(knn.model,test_transformed)

test_transformed$knn

knn.mat <- confusionMatrix(table(test_transformed$Attrition,test_transformed$knn))
#Returns "Error in !all.equal(nrow(data), ncol(data)) : invalid argument type"
knn.mat
