...
#Converts Attrition to TRUE/FALSE instead of Yes/No to fix glm warning
df$Attrition <- ifelse(df$Attrition == "Yes", TRUE, FALSE)
testset$Attrition <- ifelse(testset$Attrition == "Yes", TRUE, FALSE)

#Creates the prediction and sets the values to Yes/No
predictyn <- function(fit, test)
{
  p1 <- ifelse(predict(fit, test) > 0.5, TRUE, FALSE)
  p1
}
