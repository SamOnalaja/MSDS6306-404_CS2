...
#Job Satisifcation by Role
library(plyr)
count.jr <- count(dfc,"JobRole")
count.jr
count.js <- count(dfc,"JobSatisfaction")

dfc2 <- dfc[,c("JobRole","JobSatisfaction")] %>% group_by("JobROle")

ggplot(dfc2, aes(x=dfc2$JobRole, y=dfc2$JobSatisfaction)) + geom_bar(position = "dodge",stat="identity",colour = "blue", fill ="blue") + labs(x="Job Role", y="Job Satisfaction") +  ggtitle("Job Satisfaction by Role") + coord_flip()
