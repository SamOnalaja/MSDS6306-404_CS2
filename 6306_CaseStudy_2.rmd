...
#write.csv(cust.model, "Case2Predictions.csv")

#A function
#Does the math in making the amount into a percentage
#Also does the transformation to a table and back
...
#Gets only the rows we want
#(make sure they are always in the same order)
dfc1 <- dfc[,c("JobRole","JobSatisfaction")]
dfc3 <- makereadyforplot(dfc1)

#Plot the grouped histograms
ggplot(data = dfc3, aes(x = factor(JobRole), y=Freq, group=JobSatisfaction, fill = JobSatisfaction)) +
  geom_bar(stat = "identity", position = "dodge") +
  xlab("Job Role") +
  ylab("Percent") +
  geom_text(size=4, aes(label= paste(Freq,"%",sep="")), position=position_dodge(width=0.9), vjust=-0.25) +
